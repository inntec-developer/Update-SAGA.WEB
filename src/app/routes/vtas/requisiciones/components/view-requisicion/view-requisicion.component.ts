...
export class ViewRequisicionComponent implements OnInit {
  ...
  constructor(
    ...
    this._Route.params.subscribe(params => {
      if (params['IdRequi'] != null && params['Folio'] != null) {
        ...
        this.puro = params['puro'];
        this.imprimir = false;
        this.clienteId = params['clienteId'];
      } else {
        this._Route.queryParams.subscribe(params2 => {
          if (params2['IdRequi'] != null && params2['Folio'] != null) {
            this.RequisicionId = params2['IdRequi'];
            this.Folio = params2['Folio'];
            this.Vacante = params2['Vacante'];
            this.TipoReclutamiento = params2['TipoReclutamientoId'];
            this.puro = params2['puro'];
            this.imprimir = false;
            this.clienteId = params2['clienteId'];
          }
        });
      }
    });
    ...
    if (this.puro === '1') {
      ...
      const navigationExtras: NavigationExtras = {
        queryParams: {
          ...
          tipoReclutamientoId: this.TipoReclutamiento,
          folio: this.Folio,
          vBtra: this.Vacante,
  ...
  createPDF() {
    ...
    swal.fire({
      title: '¡Descargar Requisición formato PDF!',
      text: 'El proceso puede durar varios segundos. Por favor espere...',
      type: 'warning',
      showConfirmButton: false,
      onOpen: () => {
        swal.showLoading();
        this.serviceRequisiciones.getRequisicionPDF(this.RequisicionId).subscribe(async result => {
          if (result !== 404) {
            ...
            if (this.SelectiedSection.Encabezado) {
              ...
              doc.text('FECHA SOLICITUD: '
                ...
                + '\t FECHA CUMPLIMIENTO: ' +
                this.datePipe.transform(new Date(this.Requisicion['cumplimiento']), 'dd/MM/yyyy'), 40, row += 10);
              row += 5;
              const coordinador = this.Requisicion['coodinadore'] != null ? this.Requisicion['coodinadore'].toUpperCase() :
                'SIN COORDINADOR';
              ...
              if (this.Requisicion['asignadosN'] != null) {
                this.Requisicion.asignadosN.forEach(x => {
                  doc.text(x.nombre.toUpperCase() + ' ' + x.apellidoPaterno.toUpperCase() + ' ' +
                    x.apellidoMaterno.toUpperCase(), 40, row += 10);
                });
              } else {
            ...
            if (this.SelectiedSection.Cliente) {
              ...
              const xOffsetAe = (doc.internal.pageSize.width / 2) -
                (doc.getStringUnitWidth(ActividadEmpresa) * doc.internal.getFontSize() / 2);
              doc.setFontSize(10);
              doc.rect(20, row, 570, 15);
              ...
              const xOffsetTR = (doc.internal.pageSize.width / 2) -
              (doc.getStringUnitWidth(TipoReclutamiento) * doc.internal.getFontSize() / 2);
              doc.setFontSize(10);
              doc.rect(20, row, 570, 15);
            ...
            if (this.SelectiedSection.Direccion) {
            ...
            let r = 0;
            if (this.SelectiedSection.Camiones) {
              doc.addPage();
              row = 20;
            ...
            if (this.SelectiedSection.Contacto) {
            ...
            if (this.SelectiedSection.Requisitos) {
              const NombrePerfil = this.Requisicion['vBtra'].toUpperCase();
              const xOffsetNomP = (doc.internal.pageSize.width / 2) -
                (doc.getStringUnitWidth(NombrePerfil) * doc.internal.getFontSize() / 2);
              doc.setFontSize(10);
              doc.rect(20, row, 570, 15);
            ...
            let newPage = false;
            if (this.SelectiedSection.Horarios) {
              ...
              doc.setFontSize(8);
              doc.setTextColor(255, 0, 0);
              doc.text('OBSERVACIONES DE TIPO CONFIDENCIAL, REQUISITOS DEL CLIENTE' +
              '(Para consulta exclusiva del reclutador)', 40, row += 10);
              doc.setTextColor(0, 0, 0);
              row += 5;
              let obs = 1;
            ...
            if (this.SelectiedSection.Psicom) {
              const Psicometrias = 'PSICOMETRIAS A APLICAR ';
              const xOffsetPSI = (doc.internal.pageSize.width / 2) -
              (doc.getStringUnitWidth(Psicometrias) * doc.internal.getFontSize() / 2);
              doc.setFontSize(10);
              doc.rect(20, row, 570, 15);
            ...
            if (this.SelectiedSection.Sueldos) {
            ...
            if (this.SelectiedSection.Beneficio) {
              ...
              row += 5;
              doc.setFontSize(8);
              doc.text('TIPO CONTRATO: ' +
              this.Requisicion.tipoContrato.toUpperCase() + '\t' + this.Requisicion.tiempo.toUpperCase(), 40, row += 10);
              row += 5;
            }
            if (row >= 755 && newPage === false) {
              doc.addPage();
            ...
            if (this.SelectiedSection.DocPrest) {
              ...
              const xOffsetPres = (doc.internal.pageSize.width / 2) -
              (doc.getStringUnitWidth(Presaciones) * doc.internal.getFontSize() / 2);
              doc.setFontSize(10);
              doc.rect(20, row, 570, 15);
              ...
              const xOffsetComp = (doc.internal.pageSize.width / 2) -
              (doc.getStringUnitWidth(Competencias) * doc.internal.getFontSize() / 2);
              doc.setFontSize(10);
              doc.rect(20, row, 570, 15);
            ...
            if (this.SelectiedSection.arteRedes) {
