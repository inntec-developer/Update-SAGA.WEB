...
export class ViewdamfoComponent implements OnInit {
  ...
  createPDF() {
    ...
    let direcciones = '';
    this.damfo290.cliente.direcciones.forEach(element => {
      direcciones +=
        'Calle ' + element.calle
        + ', No. ' + element.numeroExterior + ' ' + element.numeroInterior
        + ', Colonia: ' + element.colonia
        + ', Municipio: ' + element.municipio
        + ', C.P. ' + element.codigoPostal
        + '\n';
    });
    let telefonos = '';
    this.damfo290.cliente.telefonos.forEach(element => {
      telefonos +=
        'tipo: ' + element.tipo
