...
export class AreasComponent implements OnInit, AfterContentInit {
  ...
  @ViewChild(FormatoAnexosComponent) AreasRegistradas: FormatoAnexosComponent;
  ...
  getCatalogos() {
    this._serviceCatalogos.getCatalogoForId(48).subscribe(element => {
  ...
  Remove() {
    if (!this.isActionEdit) {
      ...
      if (this.IdFormato != null && this.competencia.get('id').value != null) {
        ...
        obj['action'] = 'delete';
        this._servicePerfilR.CrudCompAreas(obj).subscribe(data => {
          if (data !== 404) {
            this.remove.emit(this.index);
            this.Add.emit(false);
          } else {
