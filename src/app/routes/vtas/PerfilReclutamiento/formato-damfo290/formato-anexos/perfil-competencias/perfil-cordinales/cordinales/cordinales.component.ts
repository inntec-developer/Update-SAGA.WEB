...
@Component({
  selector: 'app-cardinales-p',
  templateUrl: './cordinales.component.html',
  styleUrls: ['./cordinales.component.scss'],
  providers: [CatalogosService, PerfilReclutamientoService]
})
export class CordinalesComponent implements OnInit, AfterContentInit {
  ...
  getCatalogos() {
    this._serviceCatalogos.getCatalogoForId(47).subscribe(element => {
  ...
  Remove() {
    if (!this.isActionEdit) {
      ...
      if (this.IdFormato !== null && this.competencia.get('id').value !== null) {
        ...
        obj['action'] = 'delete';
        this._servicePerfilR.CrudCompCardinales(obj).subscribe(data => {
          if (data !== 404) {
            this.remove.emit(this.index);
            this.Add.emit(false);
          } else {
